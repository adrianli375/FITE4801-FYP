...
# NOTE: This code is used as the final algorithm in the US stock market.
class StockMA(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.trailingstoplosspercent = 0.08 
        # self.percent_above = 0.03
        self.volatility_n_days = 10 #number of days for future prediction
        self.past_volatility_n_days = 5
        self.volatility_constant = 0  ### New
        self.volatility_coefficient = 0.5 

        self.close_volatility_constant = 0  ### New
        ...
        self.num_days_lookback = 150
        self.adjustCloseVol = True
        ### End Parameters
    ...
    def TrainAlgo(self):
        ...
        # Second LSTM layer
        self.regressorLSTM.add(LSTM(units=128, return_sequences=True, input_shape=(x_train.shape[1],self.past_volatility_n_days)))
        self.regressorLSTM.add(Dropout(0.2))
        # Third LSTM layer
        self.regressorLSTM.add(LSTM(units=64, return_sequences=False, input_shape=(x_train.shape[1],self.past_volatility_n_days)))
        self.regressorLSTM.add(Dropout(0.2))
        # The output layer
    ...
    def RetrainAlgo(self):
        ...
        '''
        # obtains the past history
        history = self.History(self.symbol, timedelta(days=self.retrain), Resolution.Daily)
    ...
    def OnData(self, slice:Slice):
        ...
        for i in range(len(df["close"].values)):
            if i > 4:
                ...
                # pri,curmax,lastmax,lastmaxindex)
                if curmax > lastmax:
        ...
        final_MA = min(self.max_MA,final_MA) ##NEW
        ...
        self.percent_above = self.volatility_constant + y_predict[0] * self.volatility_coefficient + pnl_count * self.penalty_coefficient
        if self.adjustCloseVol:
            # penalty term added to close trades
            self.close_above = self.close_volatility_constant + y_predict[0] * self.close_volatility_coefficient + pnl_count * self.penalty_coefficient
        else:
            self.close_above = self.close_volatility_constant + y_predict[0] * self.close_volatility_coefficient
