...
# Version 3 of the algorithm in the US stock market. 
class StockMA(QCAlgorithm):
    ...
    def OnData(self, slice:Slice):
        ...
        df = self.History(self.symbol, self.volatility_n_days+1, Resolution.Daily)
        c = df['close'].pct_change().dropna().std()

        # determine the width of the MA bands based on past volatility
        self.percent_above = c * self.volatility_coefficient

        self.Log(df['close'].std()/MA)
