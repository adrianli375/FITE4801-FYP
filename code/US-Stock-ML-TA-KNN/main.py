...
class SmoothYellowGreenAlpaca(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.extendWhenSignalled = True #self.sellatndays must be True
    ...
    def OnData(self, data: Slice):
        ...
        exe_price = self.Portfolio[self.stock].AveragePrice
        ...
        if abs(q*price)>100:
            ...
            if not sell and self.sellatnpercent:
                ...
                if q>0:
                    self.LimitOrder(self.stock, -q, exe_price * (1 + self.npercent) , tag="take profit")
                else:
                    self.LimitOrder(self.stock, -q, exe_price * (1 - self.npercent) , tag="take profit")
